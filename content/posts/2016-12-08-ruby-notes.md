...
`=~`是用于正则表达式的匹配操作符。返回匹配到的字符串位置或 nil。
...
ruby 的 `for in` 也是一种迭代。We can use a control structure `retry` in conjunction with an iterated loop, and it will retry the loop from the beginning.
...
全局变量以 `$` 开头。初始化之前，全局变量的值为`nil`。可定义一段 procedure 来追踪全局变量。
