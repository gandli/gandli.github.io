...
先是 `401 invalid access token`——API Key 格式不对，阿里云的 Key 是 `sk-sp-` 开头的，不是普通的 `sk-`。接着又来一个 `403 invalid api-key`，这次是 Base URL 写错了，Anthropic 兼容接口要用 `/apps/anthropic` 结尾。
...
rm ~/.openclaw/agents/main/sessions/a85265de-*.js
